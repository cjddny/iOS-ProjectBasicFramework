...
{
    [self palyVoiceWithObj:fileName];
}
- (void)managerAudioWithData:(NSData*)amrData toPlay:(BOOL)toPlay
{
    [self palyVoiceWithObj:amrData];
}
-(void)palyVoiceWithObj:(id)obj
{
...
//        declaration of avaudiosession must be imported  from module avfoundation.avaudiosession before
    ...
    }else
    {
        ...
        if (_playingFileData && [_playingFileData isEqualToData:obj]) {//上次播放的录音
            if (_player)
            {
                [_player play];
                [[UIDevice currentDevice] setProximityMonitoringEnabled:YES];
        ...
        {//不是上次播放的录音
            ...
            /*
             ...
             AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:[XHVoiceCommonHelper getPathByFileName:fileName ofType:@"wav"]] error:nil];
             */
            AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithData:obj  error:nil];
            pl.delegate = self;
            [pl play];
            self.player = pl;
            [[UIDevice currentDevice] setProximityMonitoringEnabled:YES];
...
+ (id)shareInstance {
    static AudioPlayerHelper *instance = nil;
...
- (void)sensorStateChange:(NSNotificationCenter *)notification {
    ...
    if ([[UIDevice currentDevice] proximityState] == YES) {
        //黑屏
        KSLOGBASIC_INFO(@"Device is close to user");
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayAndRecord error:nil];
        
    } else {
        //没黑屏幕
        KSLOGBASIC_INFO(@"Device is not close to user");
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];
        if (!_player || !_player.isPlaying) {
