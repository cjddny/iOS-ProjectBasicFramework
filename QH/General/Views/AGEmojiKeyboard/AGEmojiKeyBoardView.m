...
static const CGFloat SegmentsBarHeight = 35;
...
- (NSArray *)imagesForSelectedSegments {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (AGEmojiKeyboardViewCategoryImage i = AGEmojiKeyboardViewCategoryImageFace;
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource {
    ...
    if (self) {
        ...
        UIView* linkView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, 0.5)];
        linkView.backgroundColor = [UIColor colorWithRed:47/255 green:50/255 blue:55/255 alpha:0.1];
        ...
        self.segmentsBar.frame = CGRectMake(0, 0, CGRectGetWidth(self.bounds)+60, SegmentsBarHeight);
        ...
        UIImage *separator =[UIImage imageNamed:@"PhoneSepLine"];
        ...
        UIScrollView *scrollerViewSeg = [[UIScrollView alloc]initWithFrame:CGRectMake(0, CGRectGetHeight(self.frame)-SegmentsBarHeight, kScreen_Width, SegmentsBarHeight)];
        
        scrollerViewSeg.backgroundColor = [UIColor clearColor];
        scrollerViewSeg.indicatorStyle = UIScrollViewIndicatorStyleBlack;//滚动条样式
        scrollerViewSeg.showsHorizontalScrollIndicator = NO;
        //显示横向滚动条
        scrollerViewSeg.showsVerticalScrollIndicator = NO;//关闭纵向滚动条
        scrollerViewSeg.bounces = YES;//取消反弹效果
        scrollerViewSeg.contentSize = CGSizeMake(self.segmentsBar.frame.size.width+60, self.segmentsBar.frame.size.height -1);
        if (IsAfterIOS8)
        {
            scrollerViewSeg.panGestureRecognizer.delaysTouchesBegan = scrollerViewSeg.delaysContentTouches;
        }
        [scrollerViewSeg addSubview:self.segmentsBar];
        [self addSubview:scrollerViewSeg];
        
        
        UIButton*doneButton =[[UIButton alloc]initWithFrame:CGRectMake(kScreen_Width-60, scrollerViewSeg.frame.origin.y, 60, CGRectGetHeight(self.segmentsBar.bounds))];
        [doneButton setTitle:@"完成" forState:UIControlStateNormal];
        [doneButton setBackgroundColor:colorFromHex(@"ebebeb")];
        [doneButton addTarget:self action:@selector(didDoneButtonClick) forControlEvents:UIControlEventTouchUpInside];
        [doneButton setTitleColor:colorFromHex(@"282828") forState:UIControlStateNormal];
        [doneButton bringSubviewToFront:scrollerViewSeg];
        [self addSubview:doneButton];
        
        
        //  RACSignal *kbShow = [[NSNotificationCenter defaultCenter] rac_addObserverForName:UIKeyboardWillShowNotification object:nil];
        //  @weakify(self);
        //  void (^keyboardShowHideBlock)(NSNotification *notification) = ^(NSNotification *notification) {
        ...
        //      [scrollerViewSeg setFrame:CGRectMake(0, CGRectGetHeight(keyboardEndFrame)-SegmentsBarHeight, 320, SegmentsBarHeight)];
        //      @strongify(self);
        //      [self.doneButton setFrame:CGRectMake(320-60, scrollerViewSeg.frame.origin.y, 60, CGRectGetHeight(self.segmentsBar.bounds))];
        //
        //  };
        //  [kbShow subscribeNext:keyboardShowHideBlock];
        ...
        UIView* linkView2 = [[UIView alloc]initWithFrame:CGRectMake(0, self.frame.size.height-SegmentsBarHeight, self.frame.size.width, 0.5)];
        linkView2.backgroundColor = [UIColor colorWithRed:47/255 green:50/255 blue:55/255 alpha:0.1];
        [self addSubview:linkView2];
    }
    return self;
}
-(void)didDoneButtonClick
{
